...
def batch():
    ...
    return {
        "logits_s": torch.randn(2, 3, requires_grad=True),
...
@pytest.fixture
def seg_mask_with_labels(batch):
...
class TestVisionOnlyLoss:
    ...
    def test_total_with_distillation(self, batch):
        loss = VisionOnlyLoss(cls_loss_name="cross_entropy", w_sd=0.5)
        total, parts = loss.total(batch["logits_s"], batch["logits_t"], batch["y"])

        assert "cls" in parts
        assert "sd" in parts
        assert total.requires_grad

    def test_total_without_distillation(self, batch):
        """When logits_t is None, only classification loss should be present."""
        loss = VisionOnlyLoss(cls_loss_name="cross_entropy", w_sd=0.5)
        total, parts = loss.total(batch["logits_s"], None, batch["y"])

        assert "cls" in parts
        assert "sd" not in parts

    def test_total_sd_disabled(self, batch):
    ...
    def test_gradient_flow(self, batch):
        loss = VisionOnlyLoss(cls_loss_name="cross_entropy", w_sd=0.5)
        total, _ = loss.total(batch["logits_s"], batch["logits_t"], batch["y"])
        total.backward()
        assert batch["logits_s"].grad is not None
...
class TestMultimodalLoss:
    ...
    def test_total_includes_alignment(self, batch):
        loss = MultimodalLoss(w_align=1.0, cls_loss_name="cross_entropy", w_sd=0.5)
        total, parts = loss.total(
            batch["logits_s"], batch["logits_t"], batch["y"],
            batch["f_v"], batch["f_t"]
        )
...
class TestMultitaskLoss:
    ...
    def test_total_includes_segmentation(self, batch, seg_mask_with_labels):
        loss = MultitaskLoss(w_seg=1.0, seg_loss_name="dice_ce", cls_loss_name="cross_entropy", w_sd=0.5)
        total, parts = loss.total(
            batch["logits_s"], batch["logits_t"], batch["y"],
            batch["seg_logits"], seg_mask_with_labels
        )
    ...
    def test_seg_disabled(self, batch, seg_mask_with_labels):
    ...
    def test_dice_focal_variant(self, batch, seg_mask_with_labels):
    ...
    def test_gradient_flows_to_seg(self, batch, seg_mask_with_labels):
        ...
        total.backward()
        assert batch["seg_logits"].grad is not None
